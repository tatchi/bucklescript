...
function next_char(ib) {
  try {
    var c = Curry._1(ib.get_next_char, /* () */0);
    ib.current_char = c;
    ib.current_char_is_valid = true;
    ib.char_count = ib.char_count + 1 | 0;
    if (c === /* "\n" */10) {
      ib.line_count = ib.line_count + 1 | 0;
    }
    return c;
  }
  catch (exn){
    if (exn === Caml_builtin_exceptions.end_of_file) {
      ib.current_char = /* "\000" */0;
...
function peek_char(ib) {
  if (ib.current_char_is_valid) {
    return ib.current_char;
  } else {
    return next_char(ib);
  }
}
...
function beginning_of_input(ib) {
  return ib.char_count === 0;
}

function name_of_input(ib) {
  var match = ib.input_name;
...
function char_count(ib) {
  if (ib.current_char_is_valid) {
    return ib.char_count - 1 | 0;
  } else {
    return ib.char_count;
  }
}

function token(ib) {
  var tokbuf = ib.tokbuf;
  var tok = $$Buffer.contents(tokbuf);
  tokbuf.position = 0;
  ib.token_count = ib.token_count + 1 | 0;
  return tok;
}
        ...
        if (ib$1.eof) {
          ...
          if (switcher > 4 || switcher < 0) {
            if (switcher !== 23) {
              ...
              ib$1.current_char_is_valid = false;
              continue ;
            }
...
function scan_chars_in_char_set(char_set, scan_indic, width, ib) {
  var scan_chars = function (_i, stp) {
    while(true) {
      var i = _i;
      var c = peek_char(ib);
      if (i > 0 && !ib.eof && CamlinternalFormat.is_in_char_set(char_set, c) && c !== stp) {
...
function get_counter(ib, counter) {
  switch (counter) {
    case 0 :
        return ib.line_count;
    case 1 :
        return char_count(ib);
    case 2 :
        return ib.token_count;
